...
namespace LabMenu
{
    public partial class Form3 : Form
    {
        ...
        async private void createbt_Click(object sender, EventArgs e) // Функция, описывающая все виды взаимодействия при нажатии на кнопку "Сформировать"
        {
            ...
            else if (randfreqrb.Checked == true) // При нажатии на "Случайно с частотой"
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger.WriteLog("Date" + "\r\n" + ex.Message + "\r\n" + ex.StackTrace + "\r\n" + Form.ActiveForm.ToString().Split(',')[0] + "\r\n");
                    //ex.StackTrace
                    errorProvider1.SetError(freqtb, "Введите частоту!");
                    
                    if(dbcheck.Checked == true) 
                        await MyPGSQLSender.Main(ex.Message, ex.StackTrace, DateTime.Now.ToString(), Form.ActiveForm.ToString().Split(',')[0]);
